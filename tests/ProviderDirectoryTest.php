...
class ProviderDirectoryTest extends TestCase
{
    ...
    public function setUp()
    {
        ...
        ProviderDirectoryRestClient::login(getenv('MAXMD_APIUSERNAME'), getenv('MAXMD_APIPASSWORD'));
    ...
    {
        $this->assertEquals(Domains::providerDirectory() . '/Directory/rest/getDirectory/', $this->ProviderRestClient->url());
    }
    ...
    private function setupTestVarsAndTestGetListByHisp()
    {
        ...
        while($result === null && $i < $numResults) {
            $canUse = true;
            foreach($needsToBeConfigured as $key) {
                $canUse = $canUse && strlen($results[$i][$key]) > 1 && $results[$i][$key] != 'N/A';
            }
        ...
        $this->assertHasAllColumns($result, 'Failed checking for columns with hisp ' . self::HISP_OPERATOR);
